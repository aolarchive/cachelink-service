...
module.exports = function (config, log, redis) {
	...
	function cacheGetMany(options) {
		...
		if (!options.keys || options.keys.length) {
			return Promise.reject('must provide "keys" to get');
		}
	...
	function cacheSet(options) {
		...
		if (assocLen) {
			...
			for (var i = 0; i < assocLen; i++) {
				...
				argsSetExpire.push(assocKeyContains);
			}
			asyncAssociateContains = Promise.all(waitingAssociateContains);

			// Set the TTL for all contains sets based on their contained keys' TTLs.
			argsSetExpire.push(prefixData);
			asyncExpireAssociateContains = redis('pexpiresmax', argsSetExpire);
	...
	function cacheClearNow() {
		...
		function cacheClearNowIteration() {
			...
			return redis('smpop', [clearNowSet, clearsPerIteration]).then(function (keysToClear) {

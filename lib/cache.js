...
module.exports = function buildCache(config, log, redis) {
  ...
  function cacheSet(options) {
    ...
    if (assocLen) {
      ...
      asyncAssociateIn = redis('sadd', keyIn, ...assoc);
  ...
  function cacheClearLater(options) {
    ...
    return redis('sadd', clearLaterSet, ...options.keys).then(result => ({ success: !!result, added: +result }));
  }

  /**
   * Clear all keys in the "clear now" set. This will clear in batches.
   *

...
module.exports = function (config, log, redis) {
	...
	function cacheGet(options) {
		...
		return redis('get', [key]).then(function (raw) {
	...
	function cacheGetMany(options) {
		...
		return redis('mget', keys).then(function (rawArray) {

			if (rawArray && rawArray.length !== keysLen) {
				var e = new Error(
					'cache: redis(mget) returned ' + rawArray.length + ' replies, expected ' + keys.length
				);
	...
	function cacheSet(options) {
		...
		var asyncCacheSet = redis('set', [keyData, options.data, 'PX', millis]);
	...
	function cacheClearLater(options) {
		...
		return redis('sadd', argsSetAdd).then(function (result) {
			return {
				success: !!result,
				added: +result
			};

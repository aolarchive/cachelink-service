...
module.exports = function (config, log, redis, cache) {
	...
	function startCron() {
		if (!cronInterval) {
			cronInterval = setInterval(function () {
				...
				redis('set', syncKey, clearIntervalMillis, 'px', clearIntervalMillis - 500, 'nx').then(function (success) {

					if (success) {

						log.info('cron: needs clear, starting clear process');
						startClearNowProcess();
					}
				});
			}, clearIntervalMillis);
		}
	...
	function startClearNowProcess() {
		...
		redis('smoveall	', [clearLaterSet, clearNowSet]).then(function (success) {

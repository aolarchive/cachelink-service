...
module.exports = function buildConfig(environment) {
  ...
  function envMultilineOption(envKey, required) {
    const val = env(envKey, required);
    if (val) {
      return val.replace(/\\n/g, '\n');
    }
    return val;
  ...
  const config = {
    ...
    // HTTPS configuration.
    httpsPrivateKey: envMultilineOption('CACHELINK_HTTPS_PRIVATE_KEY', false),
    httpsCertificate: envMultilineOption('CACHELINK_HTTPS_CERTIFICATE', false),
    httpsCa: envMultilineOption('CACHELINK_HTTPS_CA', false),
    httpsRequestCert: envBool('CACHELINK_HTTPS_REQUEST_CERT', true),
    httpsRejectUnauthorized: envBool('CACHELINK_HTTPS_REJECT_UNAUTHORIZED', true),
  };

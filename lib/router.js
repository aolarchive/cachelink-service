...
module.exports = function (config, log, cache, cron) {
	...
	function routeSet(req, res) {
		...
		cache.set(options).then(complete, error);
	}
	...
	function routeClear(req, res) {
		...
		cache.clear(options).then(complete, error);
	}
	...
	function routeClearLater(req, res) {
		...
		cache.clearLater(options).then(complete, error);
	}
	...
	function routeClearNow(req, res) {
		...
		cron.startClearNowProcess().then(complete, error);
	}
		...
		broadcastBaseUrls.forEach(function (baseUrl) {
			...
			request({
				...
				if (e) {
					...
					log.error('broadcast: failed ' + '(' + url + ') (status ' + status + ')', { body: body });
				} else {

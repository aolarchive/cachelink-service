...
module.exports = function (config, log, cache, cron) {
	...
	for (var i = 0; i < broadcastBaseUrls.length; i++) {
		...
		if ('string' !== typeof url) {
			throw new Error('invalid URL (must be a string) in broadcast[' + i + ']');
		}
		if (!url.match(/^https?\:\/\//)) {
			throw new Error('invalid URL (must start with "http://" or "https://") in broadcast[' + i + ']');
		}
		if (!url.match(/^https?\:\/\/[^/]+/)) {
			throw new Error('invalid URL (must not contain a path portion) in broadcast[' + i + ']');
		}
	}
